...
select distinct
    ...
    round(FIRST_VALUE(price_value) OVER (
    ...
    round(MAX(price_value) OVER (
        PARTITION BY coin_id, cast(price_timestamp_raw as date) 
    ), 2) AS high_price,
    
    -- Low: min across the partition
	round(MIN(price_value) OVER (
        PARTITION BY coin_id, cast(price_timestamp_raw as date) 
    ), 2) AS low_price, 
    round(FIRST_VALUE(market_cap_value) OVER (
        PARTITION BY coin_id, cast(price_timestamp_raw as date) 
        ORDER BY price_timestamp_raw DESC
        ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
    ), 2) AS close_market_cap,
    round(FIRST_VALUE(total_volume_value) OVER (
        PARTITION BY coin_id, cast(price_timestamp_raw as date) 
        ORDER BY price_timestamp_raw DESC
        ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING
    ), 2) AS close_total_volume
FROM unpacked